# phi3_layer.py
# Single Layer Execution for Phi-3 (Prefill & Decode)
...
if is_decode == 0.0:
    # PREFILL
    rmsnorm_GPU_1(hidden, ln1_g, h_ln1, n_seq)
    matmul_4bit_GPU_2(h_ln1, qkv_packed, qkv_scales, qkv_buf, kp_qkv, n_seq, 9216)
    rope_and_cache_GPU_2(qkv_buf, cos_cache, sin_cache, k_cache, v_cache, n_seq, 32)
    causal_attn_GPU_2(qkv_buf, k_cache, v_cache, scores_2d, attn_out, cur_n_seq_arr, n_seq, 32)
    matmul_4bit_GPU_2(attn_out, o_packed, o_scales, h_attn_buf, kp_proj, n_seq, 3072)
    residual_add_GPU_2(hidden, h_attn_buf, n_seq, 3072)

    rmsnorm_GPU_1(hidden, ln2_g, h_ln2, n_seq)
    matmul_4bit_GPU_2(h_ln2, gate_up_packed, gate_up_scales, h_ff_buf, kp_fc, n_seq, 16384)
    silu_GPU_2(h_ff_buf, n_seq, 8192)
    matmul_4bit_GPU_2(h_ff_buf, down_packed, down_scales, h_out_buf, kp_fcp, n_seq, 3072)
    residual_add_GPU_2(hidden, h_out_buf, n_seq, 3072)
else:
    # DECODE
    rmsnorm_decode_GPU_1(hidden, ln1_g, h_ln1, cur_n_seq_arr, 3072)
    matmul_4bit_decode_GPU_1(h_ln1, qkv_packed, qkv_scales, qkv_buf, kp_qkv, cur_n_seq_arr, 9216)
    rope_and_cache_decode_GPU_1(qkv_buf, cos_cache, sin_cache, k_cache, v_cache, cur_n_seq_arr, 32)
    causal_attn_k_decode_GPU_2(qkv_buf, k_cache, scores_2d, cur_n_seq_arr, n_seq, 32)
    causal_attn_softmax_decode_GPU_1(scores_2d, cur_n_seq_arr, 32)
    causal_attn_v_decode_GPU_2(scores_2d, v_cache, attn_out, cur_n_seq_arr, 96, 32)
    matmul_4bit_decode_GPU_1(attn_out, o_packed, o_scales, h_attn_buf, kp_proj, cur_n_seq_arr, 3072)
    residual_add_decode_GPU_1(hidden, h_attn_buf, cur_n_seq_arr, 3072)

    rmsnorm_decode_GPU_1(hidden, ln2_g, h_ln2, cur_n_seq_arr, 3072)
    matmul_4bit_decode_GPU_1(h_ln2, gate_up_packed, gate_up_scales, h_ff_buf, kp_fc, cur_n_seq_arr, 16384)
    silu_decode_GPU_1(h_ff_buf, cur_n_seq_arr, 8192)
    matmul_4bit_decode_GPU_1(h_ff_buf, down_packed, down_scales, h_out_buf, kp_fcp, cur_n_seq_arr, 3072)
    residual_add_decode_GPU_1(hidden, h_out_buf, cur_n_seq_arr, 3072)

max_cache_idx_f = n_seq * 3072.0 + 4.0 * 3072.0 - 1.0
max_cache_idx = 0
while max_cache_idx_f >= 1.0:
    max_cache_idx = max_cache_idx + 1
    max_cache_idx_f = max_cache_idx_f - 1.0

k_cache[max_cache_idx] = k_cache[max_cache_idx]
v_cache[max_cache_idx] = v_cache[max_cache_idx]
